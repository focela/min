...
func C(t *testing.T, f func(t *T)) {
	defer func() {
		if err := recover(); err != nil {
			_, _ = fmt.Fprintf(os.Stderr, "%v\n%s", err, rdebug.StackWithFilter([]string{pathFilterKey}))
			t.Fail()
		}
	}()
	f(&T{t})
...
func Assert(value, expect interface{}) {
	...
	var (
		strValue  = rconv.String(value)
		strExpect = rconv.String(expect)
	)
	...
	if t1 == t2 {
		panic(
			fmt.Sprintf(
				...
				rconv.String(value), t1, rconv.String(expect), t2,
			),
		)
	}
	// Value assert.
...
func AssertGT(value, expect interface{}) {
	...
	case reflect.String:
		passed = rconv.String(value) > rconv.String(expect)

	case reflect.Int, reflect.Int8, reflect.Int16, reflect.Int32, reflect.Int64:
		passed = rconv.Int(value) > rconv.Int(expect)

	case reflect.Uint, reflect.Uint8, reflect.Uint16, reflect.Uint32, reflect.Uint64:
		passed = rconv.Uint(value) > rconv.Uint(expect)

	case reflect.Float32, reflect.Float64:
		passed = rconv.Float64(value) > rconv.Float64(expect)
	}
	if !passed {
		panic(fmt.Sprintf(`[ASSERT] EXPECT %v > %v`, value, expect))
	}
...
func AssertGE(value, expect interface{}) {
	...
	case reflect.Int, reflect.Int8, reflect.Int16, reflect.Int32, reflect.Int64:
		passed = rconv.Int64(value) >= rconv.Int64(expect)

	case reflect.Uint, reflect.Uint8, reflect.Uint16, reflect.Uint32, reflect.Uint64:
		passed = rconv.Uint64(value) >= rconv.Uint64(expect)
...
func AssertIN(value, expect interface{}) {
	...
	case reflect.Slice, reflect.Array:
		expectSlice := rconv.Strings(expect)
		for _, v1 := range rconv.Strings(value) {
			result := false
...
func AssertNI(value, expect interface{}) {
	...
	case reflect.Slice, reflect.Array:
		for _, v1 := range rconv.Strings(value) {
			result := true
			for _, v2 := range rconv.Strings(expect) {
				if v1 == v2 {
					result = false
					break
				}
...
func Fatal(message ...interface{}) {
	_, _ = fmt.Fprintf(
		...
		rdebug.StackWithFilter([]string{pathFilterKey}),
	)
	os.Exit(1)
}
		...
		if rvValue.Kind() == reflect.Map {
			if rvExpect.Len() == rvValue.Len() {
				...
				for _, key := range ksValue {
					mValue[rconv.String(key.Interface())] = rconv.String(rvValue.MapIndex(key).Interface())
				}
				for _, key := range ksExpect {
					mExpect[rconv.String(key.Interface())] = rconv.String(rvExpect.MapIndex(key).Interface())
				}
...
func DataPath(names ...string) string {
	_, path, _ := rdebug.CallerWithFilter([]string{pathFilterKey})
