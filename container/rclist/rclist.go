...
// Package rclist provides most commonly used doubly linked list container which also supports concurrent-safe/unsafe switch feature.
package rclist
...
func (l *List) Join(glue string) string {
	...
	if length > 0 {
		for i, e := 0, l.list.Front(); i < length; i, e = i+1, e.Next() {
			buffer.WriteString(rcconv.String(e.Value))
			if i != length-1 {
...
func (l *List) UnmarshalValue(value interface{}) (err error) {
	...
	switch value.(type) {
	case string, []byte:
		err = json.UnmarshalUseNumber(rcconv.Bytes(value), &array)
	default:
		array = rcconv.SliceAny(value)
	}
	l.PushBacks(array)
	return err
}
