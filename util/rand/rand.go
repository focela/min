...
func N(min, max int) int {
	...
	// As `Intn` does not support negative numbers,
	// we shift the range to positive before calling Intn.
	return Intn(max-min+1) + min
}
...
// TODO: Improve performance for generating large slices.
func Perm(n int) []int {
	m := make([]int, n)
	for i := 0; i < n; i++ {
		j := Intn(i + 1)
...
// Meet randomly calculates whether the given probability `num`/`total` is met.
func Meet(num, total int) bool {
	return Intn(total) < num
}

// MeetProb randomly calculates whether the given probability is met.
