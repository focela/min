...
func StructToSlice(data interface{}) []interface{} {
	...
	case reflect.Struct:
		...
		// Note that, it uses the rcconv tag name instead of the attribute name if
		// the rcconv tag is fined in the struct attributes.
		for k, v := range rcconv.Map(reflectValue) {
...
func FillStructWithDefault(structPtr interface{}) error {
	...
	fields, err := rcstructs.Fields(rcstructs.FieldsInput{
		Pointer:         reflectValue,
		RecursiveOption: rcstructs.RecursiveOptionEmbedded,
	})
	if err != nil {
		return err
	}
		...
		if defaultValue := field.TagDefault(); defaultValue != "" {
			if field.IsEmpty() {
				field.Value.Set(reflect.ValueOf(
					rcconv.ConvertWithRefer(defaultValue, field.Value),
				))
			}
		}
	}
