...
func Scan(params interface{}, pointer interface{}, paramKeyToAttrMap ...map[string]string) (err error) {
	...
	if pointerType == nil {
		return rcerror.NewCode(rccode.CodeInvalidParameter, "parameter pointer should not be nil")
	}
...
func doScanList(
	...
	if bindToAttrName == "" {
		return rcerror.NewCode(rccode.CodeInvalidParameter, `bindToAttrName should not be empty`)
	}
	...
	for i := 0; i < arrayValue.Len(); i++ {
		...
		if len(relationDataMap) > 0 && !relationFromAttrValue.IsValid() {
			return rcerror.NewCodef(rccode.CodeInvalidParameter, `invalid relation fields specified: "%v"`, relationFields)
		}
		...
		if relationFields != "" && !relationBindToFieldNameChecked {
			...
			if !relationFromAttrField.IsValid() {
				var (
					fieldMap, _ = rcstructs.FieldMap(rcstructs.FieldMapInput{
						Pointer:         relationFromAttrValue,
						RecursiveOption: rcstructs.RecursiveOptionEmbeddedNoTag,
					})
				)
		...
		default:
			return rcerror.NewCodef(rccode.CodeInvalidParameter, `unsupported attribute type: %s`, bindToAttrKind.String())
		}
	}
