...
func doStruct(params interface{}, pointer interface{}, paramKeyToAttrMap map[string]string, priorityTag string) (err error) {
	...
	if pointer == nil {
		return rcerror.NewCode(rccode.CodeInvalidParameter, "object pointer cannot be nil")
	}

	defer func() {
		// Catch the panic, especially the reflection operation panics.
		if exception := recover(); exception != nil {
			if v, ok := exception.(error); ok && rcerror.HasStack(v) {
				err = v
			} else {
				err = rcerror.NewCodeSkipf(rccode.CodeInternalPanic, 1, "%+v", exception)
			}
		}
	}()
	...
	if v, ok := pointer.(reflect.Value); ok {
		...
		if pointerReflectKind != reflect.Ptr {
			return rcerror.NewCodef(rccode.CodeInvalidParameter, "object pointer should be type of '*struct', but got '%v'", pointerReflectKind)
		}
	...
	tagToAttrNameMap, err := rcstructs.TagMapName(pointerElemReflectValue, priorityTagArray)
	if err != nil {
		return err
	}
	...
	defer func() {
		if exception := recover(); exception != nil {
			if err = bindVarToReflectValue(structFieldValue, value, paramKeyToAttrMap); err != nil {
				err = rcerror.Wrapf(err, `error binding value to attribute "%s"`, attrName)
			}
		}
	}()
	// Directly converting.
...
func bindVarToReflectValue(
	...
	default:
		defer func() {
			if exception := recover(); exception != nil {
				err = rcerror.NewCodef(
					rccode.CodeInternalPanic,
