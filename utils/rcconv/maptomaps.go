...
func doMapToMaps(params interface{}, pointer interface{}, paramKeyToAttrMap ...map[string]string) (err error) {
	...
	if paramsKind != reflect.Array && paramsKind != reflect.Slice {
		return rcerror.NewCode(rccode.CodeInvalidParameter, "params should be type of slice, eg: []map/[]*map/[]struct/[]*struct")
	}
	var (
	...
	if paramsElemKind != reflect.Map && paramsElemKind != reflect.Struct && paramsElemKind != reflect.Interface {
		return rcerror.NewCodef(rccode.CodeInvalidParameter, "params element should be type of map/*map/struct/*struct, but got: %s", paramsElemKind)
	}
	...
	if pointerKind != reflect.Array && pointerKind != reflect.Slice {
		return rcerror.NewCode(rccode.CodeInvalidParameter, "pointer should be type of *[]map/*[]*map")
	}
	var (
	...
	if pointerElemKind != reflect.Map {
		return rcerror.NewCode(rccode.CodeInvalidParameter, "pointer element should be type of map/*map")
	}
	defer func() {
