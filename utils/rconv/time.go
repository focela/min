...
func Duration(any interface{}) time.Duration {
	...
	if !utils.IsNumeric(s) {
		d, _ := rtime.ParseDuration(s)
		return d
	}
	return time.Duration(Int64(any))
}

// RTime converts `any` to *rtime.Time.
...
// If no `format` given, it converts `any` using rtime.NewFromTimeStamp if `any` is numeric,
// or using rtime.StrToTime if `any` is string.
func RTime(any interface{}, format ...string) *rtime.Time {
	if any == nil {
		return nil
	}
	if v, ok := any.(iRTime); ok {
		return v.RTime(format...)
	}
	// It's already this type.
	if len(format) == 0 {
		if v, ok := any.(*rtime.Time); ok {
			return v
		}
	...
	if len(format) > 0 {
		for _, item := range format {
			t, err := rtime.StrToTimeFormat(s, item)
			if t != nil && err == nil {
				return t
			}
		}
	...
	if utils.IsNumeric(s) {
		return rtime.NewFromTimeStamp(Int64(s))
	} else {
		t, _ := rtime.StrToTime(s)
		return t
	}
}
