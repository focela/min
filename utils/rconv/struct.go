...
func doStructWithJsonCheck(params interface{}, pointer interface{}) (err error, ok bool) {
	...
	default:
		// The `params` might be struct that implements interface function Interface, eg: rvar.Var.
...
func doStruct(params interface{}, pointer interface{}, paramKeyToAttrMap map[string]string, priorityTag string) (err error) {
	...
	if pointer == nil {
		return rerror.NewCode(rcode.CodeInvalidParameter, "object pointer cannot be nil")
	}
	...
	defer func() {
		...
		if exception := recover(); exception != nil {
			if v, ok := exception.(error); ok && rerror.HasStack(v) {
				...
				err = rerror.NewCodeSkipf(rcode.CodeInternalPanic, 1, "%+v", exception)
			}
		}
	}()
	...
	if v, ok := pointer.(reflect.Value); ok {
		...
		if pointerReflectKind != reflect.Ptr {
			return rerror.NewCodef(rcode.CodeInvalidParameter, "object pointer should be type of '*struct', but got '%v'", pointerReflectKind)
		}
	...
	tagToAttrNameMap, err := rstructs.TagMapName(pointerElemReflectValue, priorityTagArray)
	if err != nil {
		return err
	}
	...
	defer func() {
		if exception := recover(); exception != nil {
			if err = bindVarToReflectValue(structFieldValue, value, paramKeyToAttrMap); err != nil {
				err = rerror.Wrapf(err, `error binding value to attribute "%s"`, attrName)
			}
		}
	}()
	// Directly converting.
