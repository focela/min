...
func doMapToMaps(params interface{}, pointer interface{}, paramKeyToAttrMap ...map[string]string) (err error) {
	...
	if paramsKind != reflect.Array && paramsKind != reflect.Slice {
		return rerror.NewCode(rcode.CodeInvalidParameter, "params should be type of slice, eg: []map/[]*map/[]struct/[]*struct")
	}
	var (
	...
	if paramsElemKind != reflect.Map && paramsElemKind != reflect.Struct && paramsElemKind != reflect.Interface {
		return rerror.NewCodef(rcode.CodeInvalidParameter, "params element should be type of map/*map/struct/*struct, but got: %s", paramsElemKind)
	}
	...
	if pointerKind != reflect.Array && pointerKind != reflect.Slice {
		return rerror.NewCode(rcode.CodeInvalidParameter, "pointer should be type of *[]map/*[]*map")
	}
	var (
	...
	if pointerElemKind != reflect.Map {
		return rerror.NewCode(rcode.CodeInvalidParameter, "pointer element should be type of map/*map")
	}
	defer func() {
