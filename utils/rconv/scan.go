...
func Scan(params interface{}, pointer interface{}, paramKeyToAttrMap ...map[string]string) (err error) {
	...
	if pointerType == nil {
		return rerror.NewCode(rcode.CodeInvalidParameter, "parameter pointer should not be nil")
	}
...
//
// var users []*Entity
// var userRecords   = EntityUser{Uid: 1, Name:"kyle"}
// var detailRecords = EntityUser{Uid: 1, Address: "ohio"}
...
func doScanList(
	...
	if bindToAttrName == "" {
		return rerror.NewCode(rcode.CodeInvalidParameter, `bindToAttrName should not be empty`)
	}
	...
	for i := 0; i < arrayValue.Len(); i++ {
		...
		if len(relationDataMap) > 0 && !relationFromAttrValue.IsValid() {
			return rerror.NewCodef(rcode.CodeInvalidParameter, `invalid relation fields specified: "%v"`, relationFields)
		}
		...
		if relationFields != "" && !relationBindToFieldNameChecked {
			...
			if !relationFromAttrField.IsValid() {
				var (
					fieldMap, _ = rstructs.FieldMap(rstructs.FieldMapInput{
						Pointer:         relationFromAttrValue,
						RecursiveOption: rstructs.RecursiveOptionEmbeddedNoTag,
					})
				)
		...
		default:
			return rerror.NewCodef(rcode.CodeInvalidParameter, `unsupported attribute type: %s`, bindToAttrKind.String())
		}
	}
