...
var (
	...
	argRegex             = regexp.MustCompile(`^\-{1,2}([\w\?\.\-]+)(=){0,1}(.*)$`)
)
...
func Init(args ...string) {
	...
	defaultParsedArgs, defaultParsedOptions = parseWithDefaultAlgorithm(args...)
}

// parseWithDefaultAlgorithm parses arguments using default algorithm.
func parseWithDefaultAlgorithm(args ...string) (parsedArgs []string, parsedOptions map[string]string) {
	parsedArgs = make([]string, 0)
	parsedOptions = make(map[string]string)
	for i := 0; i < len(args); {
		array := argRegex.FindStringSubmatch(args[i])
		if len(array) > 2 {
			if array[2] == "=" {
...
//
// Fetching Rules:
// 1. Command line arguments are in lowercase format, eg: aid.package.variable;
// 2. Environment arguments are in uppercase format, eg: AID_PACKAGE_VARIABLE；
