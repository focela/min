...
// Package intlog provides internal logging for Ratcatcher development usage only.
package intlog
...
func doPrint(ctx context.Context, content string, stack bool) {
	...
	if stack {
		...
		buffer.WriteString(rdebug.StackWithFilter([]string{stackFilterKey}))
	}
...
func file() string {
	_, p, l := rdebug.CallerWithFilter([]string{stackFilterKey})
