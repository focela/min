...
// Package rctrace provides convenience wrapping functionality for tracing feature using OpenTelemetry.
package rctrace
...
import (
	...
	"github.com/focela/ratcatcher/internal/cmd"
	"github.com/focela/ratcatcher/net/rcipv4"
	"github.com/focela/ratcatcher/net/rctrace/internal/provider"
	"github.com/focela/ratcatcher/text/rcstr"
...
const (
	...
	commandEnvKeyForMaxContentLogSize = "rc.rctrace.max.content.log.size" // To avoid too big tracing content.
	commandEnvKeyForTracingInternal   = "rc.rctrace.tracing.internal"     // For detailed controlling for tracing content.
)

var (
	intranetIps, _           = rcipv4.GetIntranetIpArray()
...
func init() {
	tracingInternal = rcconv.Bool(cmd.GetOptWithEnv(commandEnvKeyForTracingInternal, "true"))
	if maxContentLogSize := rcconv.Int(cmd.GetOptWithEnv(commandEnvKeyForMaxContentLogSize)); maxContentLogSize > 0 {
...
func GetBaggageMap(ctx context.Context) *rcmap.StrAnyMap {
	return NewBaggage(ctx).GetMap()
}

// GetBaggageVar retrieves value and returns a *rcvar.Var for specified key from baggage.
func GetBaggageVar(ctx context.Context, key string) *rcvar.Var {
...
func WithUUID(ctx context.Context, uuid string) (context.Context, error) {
	return WithTraceID(ctx, rcstr.Replace(uuid, "-", ""))
}
...
func WithTraceID(ctx context.Context, traceID string) (context.Context, error) {
	...
	if err != nil {
		return ctx, rcerror.WrapCodef(
			rccode.CodeInvalidParameter,
			err,
	...
	if !sc.HasTraceID() {
		var span trace.Span
		ctx, span = NewSpan(ctx, "rctrace.WithTraceID")
		defer span.End()
		sc = trace.SpanContextFromContext(ctx)
	}
