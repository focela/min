...
import (
	...
	"github.com/focela/orca/internal/consts"
	"github.com/focela/orca/internal/errors"
)

// stackInfo manages stack info of certain error.
type stackInfo struct {
	Index   int        // Index is the index of the current error in whole error stacks.
...
// formatStackInfos formats and returns error stack information as a string.
...
// formatStackLines formats and returns error stack lines as a string.
...
func loopLinesOfStackInfo(st stack, info *stackInfo, isStackModeBrief bool) {
	...
	for _, p := range st {
		if fn := runtime.FuncForPC(p - 1); fn != nil {
			...
			if isStackModeBrief {
				// Filter whole GoOrca packages stack paths.
				if strings.Contains(file, consts.StackFilterKeyForGoOrca) {
					continue
				}
			} else {
