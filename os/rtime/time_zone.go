...
func SetTimeZone(zone string) (err error) {
	...
	if err != nil {
		err = rerror.WrapCodef(rcode.CodeInvalidParameter, err, `time.LoadLocation failed for zone "%s"`, zone)
		return err
	}
...
func (t *Time) getLocationByZoneName(name string) (location *time.Location, err error) {
	...
	if location == nil {
		...
		if err != nil {
			err = rerror.Wrapf(err, `time.LoadLocation failed for name "%s"`, name)
		}
		if location != nil {
			zoneMu.Lock()
