...
func SetTimeZone(zone string) (err error) {
	...
	if err != nil {
		err = rcerror.WrapCodef(rccode.CodeInvalidParameter, err, `time.LoadLocation failed for zone "%s"`, zone)
		return err
	}
...
func (t *Time) getLocationByZoneName(name string) (location *time.Location, err error) {
	...
	if location == nil {
		...
		if err != nil {
			err = rcerror.Wrapf(err, `time.LoadLocation failed for name "%s"`, name)
		}
		if location != nil {
			zoneMu.Lock()
